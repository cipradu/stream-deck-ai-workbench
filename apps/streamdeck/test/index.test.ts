...
describe("action lifecycle, scheduler handoff, and renderer states", () => {
  ...
  it("expires a retained Kimi five-hour value at its reset deadline while preserving NO DATA", () => {
    ...
    for (const nonTarget of [
      { ...retainedUsage, providerId: "claude-code", usageWindow: "seven-day" as const },
      {
        ...retainedUsage,
        failureContext: {
  ...
  it("expires a retained Kimi seven-day value at its reset deadline while preserving NO DATA", () => {
    const now = 1_700_000_000_000;
    const resetsAtEpochMs = now + 60_000;
    const retainedUsage = {
      displayState: "stale",
      failureContext: {
        category: "no-data-yet",
        displayState: "no-data-yet",
        reasonCode: "usage-kimi-seven-day-not-returned",
        retryClass: "healthy-poll",
        safePublicMessage: "No provider data is available yet.",
      },
      failureIndicator: "NO DATA",
...
describe("usage credential refresh is single-flighted", () => {
  ...
  it("keeps each provider's refresh gate independent", async () => {
    let claudeSpawns = 0;
    let kimiSpawns = 0;
    const claudeEntered = Promise.withResolvers<void>();
    const kimiEntered = Promise.withResolvers<void>();
    const release = Promise.withResolvers<void>();
    ...
    try {
      await Promise.all([claudeEntered.promise, kimiEntered.promise]);
