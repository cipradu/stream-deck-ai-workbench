...
const providerId = "jina" as const;
const balancePattern = /Balance left\D*(\d+)/;

export const jinaBalanceProviderModule = {
  providerId,
  ...
  // credential, reads the TEXT body at the source via `requestTextBody` (ONE attempt, NO
  // retry — jina returns a plain-text balance line, not JSON), and yields the plain
  // normalized snapshot. The Effect-native scheduler consumes this adapter Effect directly (no
  // Promise bridge on the live path); the scheduler remains the single retry owner.
  createSourceFetchEffect(input: CreateBalanceProviderSourceFetchInput): EffectBalanceSchedulerFetch {
    return createBalanceSourceFetchEffect(input, {
      ...
      normalize: jinaBalanceProviderModule.normalize,
    });
  },
  normalize(input: NormalizeBalanceProviderResponseInput): BalanceProviderNormalizationResult {
    const parsed = parseBalanceResponse(Schema.String, input, "balance-jina-response-schema");
    if (!parsed.ok) {
      return parsed;
    }

    const match = balancePattern.exec(parsed.value);
    if (match === null) {
      return semanticValidationFailure(input.providerId, "balance-jina-text-pattern-missing");
    }

    const value = Number(match[1]);
    return balanceSnapshotResult(input, value);
  },
} as const;
